	...
	while(running){
		...
		if(start_ramp){
			...
			if ((regulator_state == REGULATOR_ON)&&(std::abs(currSP-current)>0)){
				...
				if(currSP>current){
					DPRINT("[%s,%d] RampingUP adccurr %d (%f) set point %f increments %f, ramp speed up %f\n",dev.c_str(),slave_id,current,current*current_sensibility,currSP*current_sensibility,ramp_speed_up*current_sensibility*update_delay/1000000,ramp_speed_up*current_sensibility);

					current+=MIN(std::abs(currSP-current),ramp_speed_up*((float)update_delay/1000000.0));

				} else if(currSP<current){
					DPRINT("[%s,%d] Ramp down adccurr %d (%f) set point %f increments %f,ramp speed down %f\n",dev.c_str(),slave_id,current,current*current_sensibility,currSP*current_sensibility,ramp_speed_down*current_sensibility*update_delay/1000000,ramp_speed_down*current_sensibility);

					current-=MIN(std::abs(currSP-current),ramp_speed_down*((float)update_delay/1000000.0));
				}
...
int SimPSupply::init(){
	...
	*buf = 0;
	...
	if(((regulator_state == REGULATOR_ON) && (polarity==0))||((min_current<0) && (polarity == 0))){
