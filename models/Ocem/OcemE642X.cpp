	...
	while(run){
		...
		if(start_ramp && (start_ramp.mod_time()>OCEM_REFRESH_TIME)){
			...
			if(inst_delta < delta_current_sp){
				DPRINT("[%s,%d] CHECK CURRENT RAMP (mod time %ld us ago) current sp %f, inst curr %f, final delta current:%f inst delta curr %f, COMMAND STARTED OK, retry command %d, retry check %d.",dev.c_str(),slave_id,start_ramp.mod_time(),current_sp,inst_curr,delta_current_sp,inst_delta,retry_current,try_check_current);
				start_ramp =0;

				delta_current_sp=0;
			} else {
				DPRINT("[%s,%d] CHECK CURRENT RAMP (mod time %ld us ago) current sp %f, inst curr %f, final delta current:%f,inst delta curr %f, retry command %d, retry check %d.",dev.c_str(),slave_id,start_ramp.mod_time(),current_sp,inst_curr,delta_current_sp,inst_delta,retry_current,try_check_current);
				start_ramp=1;
...
int OcemE642X::getPolarity(int* pol,uint32_t timeo_ms){
	...
	*pol = (polarity == POL_POS)?1:(polarity == POL_NEG)?-1:0;
	...
	if(timeo_ms && (polarity.mod_time()> timeo_ms*1000)){
		ERR("[%s,%d] Timeout of %u ms getting polarity mod time:%lu",dev.c_str(),slave_id,timeo_ms,polarity.mod_time()/1000);
...
int  OcemE642X::getCurrentOutput(float* curr,uint32_t timeo_ms){
	...
	*curr = current*adc_current_conversion;
	...
	if(timeo_ms && (current.mod_time()> timeo_ms*1000)){
		ERR("[%s,%d] Timeout of %u ms, getting current mod time:%lu.",dev.c_str(),slave_id,timeo_ms,current.mod_time()/1000);
...
int OcemE642X::getAlarms(uint64_t*alrm,uint32_t timeo_ms){
	...
	*alrm = alarms;
	...
	if(timeo_ms && (alarms.mod_time()> timeo_ms*1000)){
		ERR("[%s,%d] Timeout of %u ms getting alarms mod time:%lu",dev.c_str(),slave_id,timeo_ms,alarms.mod_time()/1000);
...
int  OcemE642X::getState(int* state,std::string &desc,uint32_t timeo_ms){
	*state = 0;
	...
	if(timeo_ms && (regulator_state.mod_time()> timeo_ms*1000)){
		ERR("[%s,%d] Timeout of %u ms getting state mod time:%lu",dev.c_str(),slave_id,timeo_ms,regulator_state.mod_time()/1000);
		//regulator_state = REGULATOR_UKN;
